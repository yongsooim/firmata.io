 ...
 */
export function boardLookupTable(): Map<string, Board> {
  var byBoard = new Map<string, Board>()
  for (var i = 0; i < boards.length; i++) {
    var currentBoard = boards[i]
    //byBoard[currentBoard.name] = currentBoard
    byBoard.set(currentBoard.name, currentBoard)

    var aliases = currentBoard.aliases
    if (Array.isArray(aliases)) {
      for (var j = 0; j < aliases.length; j++) {
        var currentAlias = aliases[j]
        byBoard.set(currentAlias, currentBoard)
      }
    }
  }
  return byBoard
